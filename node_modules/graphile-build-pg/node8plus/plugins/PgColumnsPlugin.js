...
exports.default = function PgColumnsPlugin(builder) {
  ...
  builder.hook("GraphQLInputObjectType:fields", (fields, build, context) => {
    ...
    return extend(fields, introspectionResultsByKind.attribute.filter(attr => attr.classId === table.id).filter(attr => pgColumnFilter(attr, build, context)).filter(attr => !omit(attr, isPgBaseInput ? "base" : isPgPatch ? "update" : "create")).filter(attr => attr.identity !== "a").reduce((memo, attr) => {
      const fieldName = inflection.column(attr);
      if (memo[fieldName]) {
